...
if __name__ == "__main__":
    images_dir = "images"  # Directory containing input images and keypoints
    reference_dir = "reference"  # Directory containing reference image and keypoints
    output_dir = "output"  # Directory to save output matches
    os.makedirs(output_dir, exist_ok=True)

    # Reference image and keypoints
    ref_img_path = os.path.join(reference_dir, "img_ref.jpg")
    ref_kp_file = os.path.join(reference_dir, "kp_ref.mat")

    # Load list of input images and keypoints
    image_files = sorted([f for f in os.listdir(images_dir) if f.endswith('.jpg')])
    keypoint_files = sorted([f for f in os.listdir(images_dir) if f.endswith('.mat')])

    for i, (image_file, kp_file) in enumerate(zip(image_files, keypoint_files)):
        img_path = os.path.join(images_dir, image_file)
        kp_path = os.path.join(images_dir, kp_file)
        output_path = os.path.join(output_dir, f"matches_ref_to_{i}.png")

        print(f"Processing {image_file} against reference...")
        display_matches(ref_img_path, img_path, ref_kp_file, kp_path, output_path, use_ransac=True)
