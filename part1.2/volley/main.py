...
def compute_graph(images, keypoints, descriptors, min_matches=40):
    """
...
def load_yolo_data(yolo_path):
    """
    Load YOLO bounding box data and associated metadata from a .mat file.

    Args:
        yolo_path (str): Path to the YOLO .mat file.

    Returns:
        dict: Dictionary containing class, id, and bounding box (xyxy) data.
    """
    data = loadmat(yolo_path)
    yolo_data = {
        "class": data["class"].flatten(),  # Array of class IDs
        "id": data["id"].flatten(),        # Array of object IDs
        "xyxy": data["xyxy"]              # Bounding boxes (x1, y1, x2, y2)
    }
    return yolo_data
...
def process_image_sequence_with_shortest_path(ref_dir, input_dir, output_dir):
    ...
    all_images = {"img_ref": ref_img}
    keypoints = {"img_ref": ref_kp}
    descriptors = {"img_ref": ref_desc} 

    homographies = {"img_ref": np.eye(3, dtype=np.float32)}  # Identity for reference image

    # Graph structure for shortest path tracking
    parents = {"img_ref": None}  # Track the parent image for each image
    ...
    for img_file in input_files:
        ...
        yolo_path = os.path.join(input_dir, f"yolo_{img_file[4:-4]}.mat")

        print(f"Processing {img_file}...")

        # Load input image and its keypoints
        input_img, input_kp, input_desc = load_image_and_keypoints(img_path, kp_path)
        yolo_data = load_yolo_data(yolo_path)

        # Extract bounding boxes, classes, and IDs
        bboxes = yolo_data["xyxy"]
        classes = yolo_data["class"]
        ids = yolo_data["id"]        
        
        # Draw bounding boxes on the input image
        def transform_bboxes(bboxes, H):
            """Apply homography transformation to YOLO bounding boxes."""
            transformed_bboxes = []
            for bbox in bboxes:
                x1, y1, x2, y2 = bbox
                ...
                # Find the new bounding box from the transformed corners
                transformed_x1 = np.min(transformed_corners[:, :, 0])
                transformed_y1 = np.min(transformed_corners[:, :, 1])
                transformed_x2 = np.max(transformed_corners[:, :, 0])
                transformed_y2 = np.max(transformed_corners[:, :, 1])
                transformed_bboxes.append([transformed_x1, transformed_y1, transformed_x2, transformed_y2])
            return np.array(transformed_bboxes)
        
        # Try to match directly with reference image first
        matches = find_knn_matches_with_kdtree(descriptors["img_ref"], input_desc)
        src_pts = np.float32([keypoints["img_ref"][m.queryIdx].pt for m in matches]).reshape(-1, 1, 2)
        ...
        if H is not None and mask is not None and np.sum(mask) > 40:
            # If sufficient matches with reference, add to graph
            parents[img_file] = "img_ref"
            homographies[img_file] = H
            all_images[img_file] = input_img
            keypoints[img_file] = input_kp
            descriptors[img_file] = input_desc
            
            transformed_bboxes = transform_bboxes(bboxes, H)

            # Save transformed YOLO data
            output_yolo_path = os.path.join(output_dir, f"outyolo_{img_file[4:-4]}.mat")
            savemat(output_yolo_path, {
                "xyxy": transformed_bboxes,
                "class": classes,
                "id": ids,
            })
            
            print(f"Saved transformed YOLO data to {output_yolo_path}")
        else:
            # If insufficient matches with reference, traverse the graph
            to_visit = ["img_ref"]  # Start traversal from the reference
            matched = False
            ...
            while to_visit:
                ...
                if current_parent in parents:
                    # Add all other images in the graph (children of img_ref or matched images) to the queue
                    to_visit.extend([child for child, parent in parents.items() if parent == current_parent])
            if not matched:
    ...
    if len(homographies) > 1:
        sorted_images = ["img_ref"] + [img for img in input_files if img in homographies]
